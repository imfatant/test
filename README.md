...
4) Hopefully, you now find yourself logged in to the debian user account and at the command prompt. The next task is to update and install some software using an available internet connection.
    ...
    'But wait!', you may be thinking, 'Surely I can simply use the BBBlue's WiFi to connect to my router and get internet access that way?' Absolutely, you can, but that's beacause you've used the IoT image, which has all the software necessary to do this out of the box. The downside is that it contains a lot of superfulous stuff. If you want to do this, skip ahead to Steps 13 and 14, and then do 5 through to 12.
